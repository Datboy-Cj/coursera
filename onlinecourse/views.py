...
def submit(request, course_id):
    ...
    try:
        selected_ids = {int(value) for value in request.POST.getlist("choices")}
    except (TypeError, ValueError):
        return HttpResponseBadRequest("Invalid answer selection.")
    choices = list(Choice.objects.filter(pk__in=selected_ids, question__course=course))
    if len(choices) != len(selected_ids):
        return HttpResponseBadRequest("An answer does not belong to this exam.")
    if not course.question_set.exists():
        return HttpResponseBadRequest("This course has no exam yet.")
...
def show_exam_result(request, course_id, submission_id):
    submission = get_object_or_404(
        Submission, pk=submission_id, enrollment__course_id=course_id,
        enrollment__user=request.user)
    course = submission.enrollment.course
    selected_ids = set(submission.choices.values_list("id", flat=True))
    results = []
    earned = total = 0
    for question in course.question_set.prefetch_related("choice_set").order_by("id"):
    ...
    grade = round(100 * earned / total, 1) if total else 0
    return render(request, "onlinecourse/exam_result_bootstrap.html", {
        "course": course, "submission": submission, "results": results,
        "selected_ids": selected_ids, "earned": earned, "total": total,
        "grade": grade, "passed": grade > 80,
    })
